...
function znn_quote( $atts, $content = null ) {
		return '<div class="lgn_quote">'.$content.'</div>';
}
add_shortcode( 'quote', 'znn_quote' );
/*TINY MCE Quote Button*/
function znn_add_quote_button() {  
  if ( current_user_can('edit_posts') &&  current_user_can('edit_pages') )  
 {  
   add_filter('mce_external_plugins', 'znn_add_quote_plugin');  
   add_filter('mce_buttons_3', 'znn_register_quote_button');  
  }  
}  
function znn_register_quote_button($buttons) {  
    array_push($buttons, "quote");  
    return $buttons;  
 }  
function znn_add_quote_plugin($plugin_array) {  
  $plugin_array['quote'] = get_template_directory_uri().'/js/buttons.js';  
   return $plugin_array;  
}
add_action('init', 'znn_add_quote_button');


/**FACEBOOK LIKE BUTTON **/
/**USAGE: [fblike]**/
function znn_facelike() {
...
add_shortcode('fblike', 'znn_facelike');
/*TINY Facebook Like Button*/
function znn_add_fblike_button() {  
  if ( current_user_can('edit_posts') &&  current_user_can('edit_pages') )  
 {  
   add_filter('mce_external_plugins', 'znn_add_fblike_plugin');  
   add_filter('mce_buttons_3', 'znn_register_fblike_button');  
  }  
}  
function znn_register_fblike_button($buttons) {  
    array_push($buttons, "fblike");  
    return $buttons;  
 }  
function znn_add_fblike_plugin($plugin_array) {  
  $plugin_array['fblike'] = get_template_directory_uri().'/js/buttons.js';  
   return $plugin_array;  
}
add_action('init', 'znn_add_fblike_button');


/**Youtube Video Shortcode **/
/**USAGE: [youtube width="640" height="385" video_id="EhkHFenJ3rM"]
**/
function znn_youtube_func($atts) {
...
add_shortcode('youtube', 'znn_youtube_func');
/*TINY youtube Button*/
function znn_add_youtube_button() {  
  if ( current_user_can('edit_posts') &&  current_user_can('edit_pages') )  
 {  
   add_filter('mce_external_plugins', 'znn_add_youtube_plugin');  
   add_filter('mce_buttons_3', 'znn_register_youtube_button');  
  }  
}  
function znn_register_youtube_button($buttons) {  
    array_push($buttons, "youtube");  
    return $buttons;  
 }  
function znn_add_youtube_plugin($plugin_array) {  
  $plugin_array['youtube'] = get_template_directory_uri().'/js/buttons.js';  
   return $plugin_array;  
}
add_action('init', 'znn_add_youtube_button');
...
**/
function znn_vimeo_func($atts, $content) {
...
add_shortcode('vimeo', 'znn_vimeo_func');
/*TINY Vimeo Button*/
function znn_add_vimeo_button() {  
  if ( current_user_can('edit_posts') &&  current_user_can('edit_pages') )  
 {  
   add_filter('mce_external_plugins', 'znn_add_vimeo_plugin');  
   add_filter('mce_buttons_3', 'znn_register_vimeo_button');  
  }  
}  
function znn_register_vimeo_button($buttons) {  
    array_push($buttons, "vimeo");  
    return $buttons;  
 }  
function znn_add_vimeo_plugin($plugin_array) {  
  $plugin_array['vimeo'] = get_template_directory_uri().'/js/buttons.js';  
   return $plugin_array;  
}
add_action('init', 'znn_add_vimeo_button');


/**Custom Button Shortcode **/
/**USAGE: [button class="violet"][/button]
**/
function znn_button_func($atts, $content = null) {
...
add_shortcode('button', 'znn_button_func');
/*TINY buttons Button*/
function znn_add_button_button() {  
  if ( current_user_can('edit_posts') &&  current_user_can('edit_pages') )  
 {  
   add_filter('mce_external_plugins', 'znn_add_button_plugin');  
   add_filter('mce_buttons_3', 'znn_register_button_button');  
  }  
}  
function znn_register_button_button($buttons) {  
    array_push($buttons, "button");  
    return $buttons;  
 }  
function znn_add_button_plugin($plugin_array) {  
  $plugin_array['button'] = get_template_directory_uri().'/js/buttons.js';  
   return $plugin_array;  
}
add_action('init', 'znn_add_button_button');
